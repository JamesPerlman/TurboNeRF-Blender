...
class BlenderNeRFSelectCamerasInsideRadiusOperator(bpy.types.Operator):
    ...
    def execute(self, context):
        NeRFScene.select_cameras_inside_radius(bpy.context.scene.nerf_train_panel_settings.camera_selection_radius)
        return {'FINISHED'}
...
class BlenderNeRFSelectCamerasOutsideRadiusOperator(bpy.types.Operator):
    ...
    def execute(self, context):
        NeRFScene.select_cameras_outside_radius(bpy.context.scene.nerf_train_panel_settings.camera_selection_radius)
        return {'FINISHED'}
...
class BlenderNeRFUpdateCameraImagePlaneVisibilityOperator(bpy.types.Operator):
    ...
    def execute(self, context):
        NeRFScene.update_image_plane_visibility_for_all_cameras(
            force_visible=context.scene.nerf_train_panel_settings.get_should_force_image_plane_visibility()
        )
        return {'FINISHED'}
